...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      ...
      p.complete(Try { fs.map { future => Await.result(future, 1 minute) } })
      p.future
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      ...
      for (future <- fs) yield {
        future.onComplete {
          case tryValue @ Success(_) if (!p.isCompleted) =>
            p.complete(tryValue)
          case Failure(exception) if (!p.isCompleted) =>
            p.failure(exception)
          case _ =>
        }
      }
    ...
    def delay(t: Duration): Future[Unit] = {
      val p = Promise[Unit]()
      Future { blocking { Thread.sleep(t toMillis) } }.onComplete { _ => p.success() }
      p.future
    }
     ...
     * Starts an asynchronous computation f taking a CancellationToken and returns a subscription that cancels that CancellationToken:
     */
